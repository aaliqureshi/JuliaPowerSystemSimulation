...
# sol = solve(prob, Rosenbrock23(), callback=cbs, tstops=tstop1, dt = 500e-6, adaptive = false)
...
# plotlyjs()
# plot(sol.t,v1_mag,label="DP (Rodas5P)",legend=:bottomright)
# plot(v1_mag,label="DP (Rosenbrock23)",legend=:bottomright)
...
v1_mag_fft = fft(v1_mag)

v1_mag_fft_abs = abs.(v1_mag_fft)

v1_mag_fft_phase = angle.(v1_mag_fft)
...
plotlyjs()
plot!(freqs, 20*log10.(v1_mag_fft_abs), xlabel="Frequency (Hz)", ylabel="Magnitude (dB)",
title="V1 FFT [fs=1/500e-6]",label="Rodas5P")